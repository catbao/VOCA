...
router.get('/getexperiment', getexperiment);
router.get('/getfunctions', getfunctions);

router.get('/getaggregates', getaggregates);
router.get('/gettables', gettables);
router.get('/getcolumns', getcolumns);
router.get('/start', start);
router.get('/start_progressive', start_progressive);
...
async function Start_Multi_Compute_progressive(segmentTrees,screen_m4,func, mode, parallel, width,height,errorBound, isFirst){
    let M4_array;
    // if(isFirst == true){
    M4_array = screen_m4.M4_array
    // }
    // else{
    //     M4_array = screen_m4_array[0].M4_array
    // }
    //console.time('initM4');
    await initM4(segmentTrees,M4_array,func, mode, parallel, screen_m4)
    //console.timeEnd('initM4');
    ...
    let progressive = [];
    // progressive.push(parse(stringify(screen_m4)));
    // errorBound = 0.2;
    ...
    while(computedCount < M4_array.length*2 ){
        ...
        if(isFirst){
            if(screen_m4.errorBound == 0.2 && errorBoundSatisfy(screen_m4, width,height,screen_m4.errorBound)){
                console.log("进入errorBound == 0.2");
                finalCompute(screen_m4, func);
                screen_m4.dataReductionRatio = 1 - segmentTrees[0].nodeCount*2 / segmentTrees[0].realDataNum
                progressive.push(parse(stringify(screen_m4)));
                screen_m4.errorBound = Math.floor(screen_m4.preError / errorBound) * errorBound;
                // screen_m4_array = [];
                // screen_m4_array.push(parse(stringify(screen_m4)));
                break;
            }
        }
        else{
            if(errorBoundSatisfy(screen_m4, width,height,screen_m4.errorBound)){
                console.log("进入errorBound == ", screen_m4.preError);
                finalCompute(screen_m4, func);
                // screen_m4.dataReductionRatio = 1 - segmentTrees[0].nodeCount*2 / segmentTrees[0].realDataNum
                // screen_m4.errorBound = 0.04;
                
                // screen_m4_array = [];
                // screen_m4_array.push(parse(stringify(screen_m4)));
                // let flag = errorBoundSatisfy(screen_m4, width,height,screen_m4.errorBound);
...
function calBetweenIntervals(intervalArrays, func, screen_m4){


    //console.log(intervalArrays)

    let res = []

    for(let i=0;i<intervalArrays[0].length;i++){
        let currentNodes = [];
        for(k=0;k<intervalArrays.length;k++){
            let node = new SegmentTreeNode();
            let r = intervalCalculation(intervalArrays[k][i], screen_m4.func)
            node.min = r;
            node.max = r;
        ...
        let { tmpmin: tmpmin1, tmpmax: tmpmax1 } = unifiedCalulate(null, currentNodes, func, null, true);
        let interval = new Interval(intervalArrays[0][i].start_time, intervalArrays[0][i].end_time)
        // interval.ave=tmpmin1
        // interval.sum=tmpmin1*(interval.end_time-interval.start_time+1)
        if(screen_m4.func.funName == 'ave'){
            interval.ave=tmpmin1
            interval.sum=tmpmin1*(interval.end_time-interval.start_time+1)
        }else if(screen_m4.func.funName == 'sum'){
            interval.sum = tmpmin1
            interval.ave=tmpmin1/(interval.end_time-interval.start_time+1)
        }
        res.push(interval)
    }
...
async function aggregateCalculation(screen_m4, segmentTrees, func,width,height, mode, symble, parallel,errorBound){
    ...
    console.log(screen_m4.screenStart, screen_m4.screenEnd, screen_m4.intervalLength)
...
async function computeMultyOrSingle_progressive(table,dataCount,columns, func, width,height, mode, symble, parallel, errorBound,screenStart,screenEnd,screen_m4, isFirst){
    ...
    // console.log(tables, func, width,height, mode, symble, parallel, errorBound,screenStart,screenEnd)
     ...
     //经过上面的while循环后，确定了所有需要计算的节点，保存在每个的M4.innerNode,表示在M4像素列里的node，这些node是需要进行计算的
     //按symble运算符or函数，对按照SegmentTree1,SegmentTree2树结构，对M4_array中的节点进行计算，得到。
     console.log("Start_Multi_Compute前的errorBound:", screen_m4.errorBound);
     let progressive;
     if(isFirst == true)
         progressive = await Start_Multi_Compute_progressive(segmentTrees,screen_m4,func, mode, parallel, width,height,errorBound, isFirst)
     else
         progressive = await Start_Multi_Compute_progressive(screen_m4.segmentTrees,screen_m4, func, mode, parallel, width,height,errorBound, isFirst)
     console.log("progressive:", progressive);
     console.log("progressive end");
...
async function ours(table_name,dataCount,columns,symble,params,width,height,mode,parallel,errorBound,startTime,endTime, interact_type,screen_m4, estimateType){
    ...
    screen_m4.min_values = min_values
    screen_m4.max_values = max_values
...
async function ours_progressive(table_name,dataCount,columns,symble,params,width,height,mode,parallel,errorBound,startTime,endTime, interact_type,screen_m4, isFirst){
    ...
    let progressive = await computeMultyOrSingle_progressive(table,dataCount,columns, funInfo, width,height, mode, symble, parallel, errorBound,startTime,endTime,screen_m4, isFirst)
    //let M4_array = await computeMultyOrSingle([table1], funInfo, width, 'single', symbol, parallel)
    ...
    for(let i=0;i<progressive.length;i++){
        M4_arrays.push(progressive[i].M4_array);
        min_values.push(progressive[i].exactMin);
        max_values.push(progressive[i].exactMax);

        progressive[i].M4_arrays = M4_arrays;
        progressive[i].min_values = min_values;
        progressive[i].max_values = max_values;

        M4_arrays = [];
        min_values = [];
        max_values = [];
    }
    // const jsonString = stringify(progressive);
    // // 将 JSON 字符串写入文件
    // fs.writeFile('/Users/bao/Desktop/progressive.json', jsonString, 'utf8', (err) => {
    //     if (err) {
    //         console.error('写入文件失败:', err);
    //     } else {
    //         console.log('progressive 数组已成功写入 progressive.json 文件');
    //     }
    // });
...
async function Case1(table_name,columns,symbol,params,width,height,mode,parallel,errorBound,startTime,endTime, interact_type,screen_m4){
 ...
 async function Case2(table_name,columns,symbol,params,width,height,mode,parallel,errorBound,startTime,endTime, interact_type,screen_m4){
    ...
    screen_m4.M4_arrays.push(M4_array)
    screen_m4.min_values.push(MIN)
    screen_m4.max_values.push(MAX)
...
//==============================functions end，以下是server代码
async function getexperiment(req, res){

    res.send({
        experiments: ["ours","case1","case2","case3","case4","case5", "case6", "case7"]
    })
}
...
async function getcolumns(req, res){
  ...
  WHERE table_schema = 'public' AND table_name = '${table_name}' and column_name like 'v%' order by CAST(SUBSTRING(column_name, 2) AS INTEGER);
`
    console.log(sql)
    let result = await pool.query(sql)
    ...
    for(let i=0;i<result.rows.length;i++){
        let column = result.rows[i].column_name
        columns.push(column)
    }
...
async function gettables(req, res){
    ...
    let sql = "SELECT table_name FROM information_schema.tables WHERE table_schema='public' and table_name like '%_om3' order by table_name;"
    console.log(sql)
    let result = await pool.query(sql)
    //console.log(result)
    ...
    for(let i=0;i<result.rows.length;i++){
        ...
        let table_name = result.rows[i].table_name
        if(table_name.endsWith('_om3')){
            table_name = removeEndChar(table_name, '_om3');
        }
...
class M4_Client{
    constructor(i,st,sv,min,max,et,ev,timestamp){
        this.i=i
        this.st = st;
        this.sv = sv;
        this.min = min;
...
async function start(req, res){
    ...
    let parallel = Number(req.query['parallel'])
    let errorBound = Number(req.query['errorBound'])

    let dataCount = await getCount(table_name)

    let aggregate = req.query['aggregate']
    let ag = aggregate.trim().split('#')
    let agFunc = new FunInfo()
    if(aggregate == ''){
        agFunc = null
    }else if(ag.length > 1){
        agFunc.funName = ag[0]
    ...
    console.log(table_name,dataCount,columns,symbol,'',width,height,mode,parallel,errorBound,startTime,endTime, interact_type, aggregate, agFunc)
    ...
    screen_m4.columns = [...columns]
    screen_m4.func = agFunc

    screen_m4.screenStart = Number(startTime)
    screen_m4.screenEnd = Number(endTime)
    genDatainfo(screen_m4)
...
// console.log({
//     M4_arrays: screen_m4.M4_arrays,
//     min_values: screen_m4.min_values,
//     max_values: screen_m4.max_values
// })
...
for(let i=0;i<screen_m4.M4_arrays.length;i++){
    let M4_array = []
    for(j=0;j<screen_m4.M4_arrays[i].length;j++){
        let m4 = new M4_Client(
            j
            ,screen_m4.M4_arrays[i][j].start_time
            ,screen_m4.M4_arrays[i][j].st_v
            ,screen_m4.M4_arrays[i][j].min
            ,screen_m4.M4_arrays[i][j].max
            ,screen_m4.M4_arrays[i][j].end_time
            ,screen_m4.M4_arrays[i][j].et_v
            ,screen_m4.screenStartTimestamp + (screen_m4.M4_arrays[i][j].start_time-screen_m4.screenStart)*screen_m4.delta
        )
        if(j == 599){
            console.log("测试stamp:", screen_m4.screenStartTimestamp, screen_m4.M4_arrays[i][j].start_time, screen_m4.delta)
            console.log("测试stamp:", m4.timestamp)
        }
        M4_array.push(m4)
        min = Math.min(min, screen_m4.M4_arrays[i][j].min)
        max = Math.max(max, screen_m4.M4_arrays[i][j].max)
    }
    M4_arrays.push(M4_array)
}
...
async function start_progressive(req, res){
    console.log('start')
    ...
    let specifiedErrorInit = 0.2;
    let screen_m4 = new SCREEN_M4(experiment,datasetname,0,symbolName,width,height,specifiedErrorInit)
    // screen_m4.errorBound = 0.2
    screen_m4.func = agFunc
    genDatainfo(screen_m4)
    ...
    // let screen_m4_array = [];
    let isFirst = true;
    // screen_m4_array.push(screen_m4);
    
    let currentIndex = 0;
    for(currentIndex=0; currentIndex<10; currentIndex++){
        // if (currentIndex >= 3) {
        if(screen_m4.errorBound == 0){
        ...
        switch(experiment){
            case 'ours':
                startTime = Number(startTime)
                endTime = Number(endTime)
                progressive = await ours_progressive(table_name, dataCount, columns,symbol,'',width,height,mode,parallel,errorBound,startTime,endTime, interact_type,screen_m4, isFirst)
                break; 
        }
        timeend('totaltime');
        console.log("progressive:", progressive.length);
        let M4_arrays = [];
        let M4_arrays_progressive = [];
        for(let len = 0; len<progressive.length;len++){
            // screen_m4 = progressive[len];
            for(let i=0;i<progressive[len].M4_arrays.length;i++){
                let M4_array = []
                for(j=0;j<progressive[len].M4_arrays[i].length;j++){
                    let m4 = new M4_Client(
                        j
                        ,progressive[len].M4_arrays[i][j].start_time
                        ,progressive[len].M4_arrays[i][j].st_v
                        ,progressive[len].M4_arrays[i][j].min
                        ,progressive[len].M4_arrays[i][j].max
                        ,progressive[len].M4_arrays[i][j].end_time
                        ,progressive[len].M4_arrays[i][j].et_v
                        ,screen_m4.screenStartTimestamp + (screen_m4.M4_arrays[i][j].start_time-screen_m4.screenStart)*screen_m4.delta
                    )
                    if(j == 599){
                        console.log("测试stamp:", progressive[len].screenStartTimestamp, progressive[len].M4_arrays[i][j].start_time, progressive[len].delta)
                        console.log("测试stamp:", m4.timestamp)
                    }
                    M4_array.push(m4)
                }
            ...
            M4_arrays_progressive.push(M4_arrays);
        }
        const payload = {
            ...
            // min_values: screen_m4.min_values,
            // max_values: screen_m4.max_values,
            preError: progressive[0].preError,
            M4_arrays: M4_arrays_progressive[0],
            min_values: progressive[0].min_values[0],
            max_values: progressive[0].max_values[0],
        };
...
function getAllMultiLineClassAndLinesInfo(req, res){
    ...
    const sql = `select table_schema||'.'||table_name as table_fullname from information_schema."tables" where table_type = 'BASE TABLE' and table_schema not in ('pg_catalog', 'information_schema');`
    const query = req.query;
    const userCookie = req.headers['authorization'];
    let currentPool = pool
    // if (query.mode === 'Custom') {
